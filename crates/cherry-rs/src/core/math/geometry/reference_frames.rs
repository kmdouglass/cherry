...
pub struct Cursor {
    ...
    /// Cumulative path length along the beam from the origin surface.
    track: Float,
}
...
mod test {
    ...
    #[test]
    fn cursor_rotation_matrix_after_reflect() {
        // After reflecting off a surface tilted 30 degrees (normal in YZ plane),
        // the cursor basis vectors are:
        //   right   = (-1,  0,  0)
        //   up      = ( 0, 0.5, √3/2)
        //   forward = ( 0, √3/2, -0.5)
        //
        // The rotation_matrix() must be global→cursor, meaning R*v projects v
        // onto each cursor axis. Basis vectors must be rows, not columns.
        let mut cursor = Cursor::new(0.0);
        cursor.reflect(&Vec3::new(0.0, 0.5, -(3 as Float).sqrt() / 2.0));

        let r = cursor.rotation_matrix();

        // Row 0 should equal `right`
        let right = cursor.right;
        assert!(
            (r.e[0][0] - right.x()).abs() < 1e-6
                && (r.e[0][1] - right.y()).abs() < 1e-6
                && (r.e[0][2] - right.z()).abs() < 1e-6,
            "Row 0 should be `right` (global→cursor convention)"
        );

        // Row 1 should equal `up`
        let up = cursor.up;
        assert!(
            (r.e[1][0] - up.x()).abs() < 1e-6
                && (r.e[1][1] - up.y()).abs() < 1e-6
                && (r.e[1][2] - up.z()).abs() < 1e-6,
            "Row 1 should be `up` (global→cursor convention)"
        );

        // Row 2 should equal `forward`
        let fwd = cursor.forward;
        assert!(
            (r.e[2][0] - fwd.x()).abs() < 1e-6
                && (r.e[2][1] - fwd.y()).abs() < 1e-6
                && (r.e[2][2] - fwd.z()).abs() < 1e-6,
            "Row 2 should be `forward` (global→cursor convention)"
        );

        // R must be orthogonal: R * R^T = I
        let rrt = r * r.transpose();
        assert!(
            rrt.approx_eq(&Mat3x3::identity(), 1e-6),
            "rotation_matrix() * its transpose should be identity, got: {:?}",
            rrt
        );
    }
}
