...
use crate::ray_tracing::sequential_model;

// Old interface
#[wasm_bindgen]
#[derive(Debug)]
pub struct SystemModel {
    surfaces: Vec<sequential_model::Surface>,
}
...
impl SystemModel {
    ...
    pub fn pushSurfRefrCircConic(&mut self, axial_pos: f32, diam: f32, n: f32, roc: f32, k: f32) {
        let surf = sequential_model::Surface::new_refr_circ_conic(axial_pos, diam, n, roc, k);
    ...
    pub fn pushSurfRefrCircFlat(&mut self, axial_pos: f32, diam: f32, n: f32) {
        let surf = sequential_model::Surface::new_refr_circ_flat(axial_pos, diam, n);
        self.surfaces.push(surf);
    }

    /// Insert an object or image plane into the optical system.
    pub fn pushSurfObjOrImgPlane(&mut self, axial_pos: f32, diam: f32) {
        let surf = sequential_model::Surface::new_obj_or_img_plane(axial_pos, diam);
        self.surfaces.push(surf);
    }
...
mod test {
    ...
    fn test_ray_trace_planoconvex_lens() {
        ...
        let obj_surf = sequential_model::Surface::new_obj_or_img_plane(f32::NEG_INFINITY, diameter);
        let surf1 = sequential_model::Surface::new_refr_circ_flat(surf1_axial_pos, diameter, n);
        let surf2 = sequential_model::Surface::new_refr_circ_conic(
        ...
        let img_surf = sequential_model::Surface::new_obj_or_img_plane(surf1_axial_pos + thickness + efl, diameter);
