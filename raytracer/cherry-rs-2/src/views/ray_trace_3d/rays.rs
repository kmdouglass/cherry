...
impl Ray {
    ...
    // recomputing it.
    pub fn redirect(&mut self, surf_1: &Surface, surf_2: &Surface, norm: Vec3) {
        // Do not match on the wildcard "_" to ensure that this function is updated when new
        // surfaces are added
        match surf_2 {
            // Refracting surfaces
            Surface::RefractingCircularConic(_) | Surface::RefractingCircularFlat(_) => {
    ...
    pub fn fan(n: usize, r: Float, theta: Float, z: Float, phi: Float, radial_offset_x: Float, radial_offset_y: Float, field_id: usize) -> Vec<Ray> {
    ...
    pub(crate) fn sq_grid_in_circ(radius: Float, spacing: Float, z: Float, phi: Float, radial_offset_x: Float, radial_offset_y: Float, field_id: usize) -> Vec<Ray> {
